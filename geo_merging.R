...
## select all xlsx files and load them
xfiles <- lf[grepl(".xlsx", lf)]
list_df <- list()
for (i in seq_along(xfiles)) {
  list_df[[i]] <- read.xlsx(paste0(temp, xfiles[i]), na.strings = NA)
}
names(list_df) <- str_remove_all(xfiles, ".xlsx")
rm(i)
gc()

## determine the master dataset and choose max distance for the buffer - REPLACE
master_set <- ""
d_max <- 26
...
write.xlsx(result, "comparison_result.xlsx")
gc()

mapview(list_df$emit, legend = FALSE)+
  mapview(list_df$contr, alpha = 1, color = "red")+
  mapview(list_df$houses, alpha = 2, color = "green")
